...
/** @brief Peek first element from LWQ
    ...
    @retval 0U                       LWQ is empty
    @retval First element in queue   LWQ is non-empty

    @post None

    @trace #BRCM_SWREQ_LWQ

    @limitations The API is only called when LWQ is non-empty
*/
...
/** @brief Peek last element from LWQ
    ...
    @retval 0U                      LWQ is empty
    @retval Last element in queue   LWQ is non-empty
...
/** @brief Pop an object from LWQ
    ...
    @retval     #BCM_ERR_OK             Success
    @retval     #BCM_ERR_NOMEM          Queue is full
    @retval     #BCM_ERR_INVAL_PARAMS   aObject is NULL
    @retval     #BCM_ERR_INVAL_PARAMS   aBuffer is NULL
