...
#define BRCM_SWARCH_MCU_SM_FAULT_TYPE                 (0x8738U)    /**< @brief #MCU_SMFaultType             */
#define BRCM_SWARCH_MCU_SET_SM_FAULT_CTRL_PROC        (0x8739U)    /**< @brief #MCU_SetSMFaultCtrl          */
#define BRCM_SWARCH_MCU_GET_SM_FAULT_STATUS_PROC      (0x873AU)    /**< @brief #MCU_GetSMFaultStatus        */
/** @} */
...
#define MCU_RESETREQ_LOCAL      (0x2UL) /**< @brief Perform local CPU reset (does not reset the peripherals and
                                                    memories)*/
#define MCU_RESETREQ_HSM_GLOBAL (0x3UL) /**< @brief Perform a full HSM susbsytem reset (can only
                                                    be triggered by the app CPU) */
/** @} */
...
#define MCU_LPMMODE_DEEPSLEEP     (0x7UL) /**< @brief DeepSleep mode */
/** @} */
...
/**
    @brief MCU System Monitor Fault Configuration

    @trace #BRCM_SWREQ_MCU_KERNEL_INTERFACE
 */
typedef struct sMCU_SMFaultType {
    MCU_TimeoutFaultType timeoutFault;          /**< @brief Time out faults      */
    MCU_MemoryFaultType  memoryFault;           /**< @brief Memory faults        */
    MCU_MiscFaultType    miscFault;             /**< @brief Miscellaneous faults */
} MCU_SMFaultType;
...
/** @brief Initialize MCU
    ...
    @param[in]      aBootMode   Current boot mode of device
...
*/
extern void MCU_Init(const MCU_ConfigType *const aConfig, BCM_BootModeType aBootMode);
...
/** @brief Set Low power mode of the MCU
    ...
    This API puts the MCU in low power mode.
...
/** @brief Set system monitor fault configuration

    This API configures the system monitor fault in the hardware register.

    @behavior Sync, Re-entrant

    @pre None

    @param[in]     aFaultCfg   Pointer to system monitor fault configuration

    Return values are documented in reverse-chronological order
    @retval     #BCM_ERR_OK             System monitor configured
    @retval     #BCM_ERR_INVAL_PARAMS   aFaultCfg is NULL
...
*/
extern int32_t MCU_SetSMFaultCtrl(const MCU_SMFaultType *const aFaultCfg);

/** @brief Get system monitor fault status

    This API is used to get the system monitor fault status from the hardware register.

    @behavior Sync, Re-entrant

    @pre None

    @param[out]     aFaultStatus   Pointer to system monitor fault status

    Return values are documented in reverse-chronological order
    @retval     #BCM_ERR_OK             System monitor fault status filled
    @retval     #BCM_ERR_INVAL_PARAMS   aFaultStatus is NULL
    ...
    @limitations This is a privileged API and can be called only when processor
                 is running in privileged mode.
*/
extern int32_t MCU_GetSMFaultStatus(MCU_SMFaultType *const aFaultStatus);
...
/**
    ...
    @trace #BRCM_SWREQ_MCU_INIT
 */
extern const uint32_t MCU_FlashClkTblSz;
#endif /* __BCM8958X__ */
