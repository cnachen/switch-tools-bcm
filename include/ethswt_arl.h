...
#define BRCM_SWARCH_ETHSWT_FIND_PORTMASK_FROM_ARL_PROC          (0x8317U)    /**< @brief #ETHSWT_FindPortMaskFromArl         */
#define BRCM_SWARCH_ETHSWT_GETARLCOUNT_PROC                     (0x8318U)    /**< @brief #ETHSWT_GetARLCount                 */
/** @} */
...
int32_t ETHSWT_GetARLTable(ETHSWT_HwIDType aSwtID,
        ETHSWT_ARLEntryV2Type *const aARLTbl,
        uint16_t *const aTblSize,
        uint16_t const aARLTblOffset);
...
int32_t ETHSWT_AddARLEntry(ETHSWT_HwIDType aSwtID,
        ETHSWT_ARLEntryV2Type *const aARLEntry);
...
/** @brief Get PortMask from Vlan Id and MAC Address

    This API retreives Ports which are participating
    in an ARL Entry
    ...
    @param[in]      aSwtID                   Switch index
    @param[inout]   aARLEntry                Pointer to ARL Entry

    Return values are documented in reverse-chronological order
    @retval     #BCM_ERR_OK             Successfully retrieve the PortMask
                                        participating in an ARL entry
    @retval     #BCM_ERR_INVAL_PARAMS   (Invalid switch index) or
                                        (aARLEntry is NULL)
    @retval     #BCM_ERR_NOT_FOUND      Failed to retrieve Portmask
                                        from ARL entry
...
int32_t ETHSWT_FindPortMaskFromArl(ETHSWT_HwIDType aSwtID,
                                   ETHSWT_ARLEntryV2Type *const aARLEntry);

/** @brief Get the ARL count from HW

    This API read the complete ARL table from the switch return the count
    ...
    @param[in]      aSwtID      Switch index
    @param[out]     aTblSize    Count of ARL entries

    Return values are documented in reverse-chronological order
    @retval     #BCM_ERR_OK             Successfully retrieved ARL count
    @retval     #BCM_ERR_TIME_OUT       Switch driver read-write time-out
    @retval     #BCM_ERR_UNINIT         Switch driver not initialized
    @todo       #BCM_ERR_INVAL_PARAMS   Invalid switch index
...
int32_t ETHSWT_GetARLCount(ETHSWT_HwIDType aSwtID,
        uint16_t *const aTblSize);
