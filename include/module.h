...
#define BRCM_SWARCH_MODULE_GET_TRIG_PERIOD_TYPE     (0x820BU)    /**< @brief #MODULE_GetTrigPeriodType      */
#define BRCM_SWARCH_MODULE_TYPE                     (0x820CU)    /**< @brief #MODULE_Type                   */
#define BRCM_SWARCH_MODULE_CONTEXT_TYPE             (0x820DU)    /**< @brief #MODULE_ContextType            */
#define BRCM_SWARCH_SERVICE_TYPE                    (0x820EU)    /**< @brief #SERVICE_Type                  */
#define BRCM_SWARCH_MODULE_ALARM_TYPE               (0x820FU)    /**< @brief #MODULE_AlarmStatType          */
#define BRCM_SWARCH_MODULE_GET_RESP_V2_TYPE         (0x8210U)    /**< @brief #MODULE_GetRespV2Type          */
#define BRCM_SWARCH_MODULE_NOTIFYEVENTHANDLER_TYPE  (0x8211U)    /**< @brief #MODULE_NotifyEventHandlerType */
#define BRCM_SWARCH_SERVICE_COUNT_MAX_MACRO         (0x8212U)    /**< @brief #SERVICE_COUNT_MAX             */
#define BRCM_SWARCH_SERVICE_LIST_GLOBAL             (0x8213U)    /**< @brief #SERVICE_List                  */
/** @} */
...
*/
#ifndef MODULE_COUNT_MAX
#define MODULE_COUNT_MAX  (16UL)
#endif

/**
    @name Module alarm status
    @{
    @brief Module alarm status

    @trace #BRCM_SWREQ_MODULE
*/
typedef uint32_t MODULE_AlarmStatType;
#define MODULE_ALARM_DISABLED       (0UL)
#define MODULE_ALARM_INITIAL_CYCLE  (1UL)
#define MODULE_ALARM_ENABLED        (2UL)
/** @} */
...
/** @brief Send a command to the module
    ...
    @trace #BRCM_SWREQ_MODULE
*/
typedef int32_t (*MODULE_SendCmdType)(BCM_MsgType    aCmd,
...
typedef int32_t (*MODULE_GetRespType)(BCM_HandleType aHandle,
                                    BCM_MsgType aReplyID,
                                    uint32_t aInLen,
                                    uint8_t *aReplyData,
                                    uint32_t *aReplyDataLen);

/** @brief Get response (V2) from the module for a given command

    This API validates the handle and retrieves the response.
    This API gets executed in the context of a client task (not same as state
    handler or event handler or process cmd).
    This V2 API additionally provides for an OUT argument for module to
    return the command processsing response.
    ...
    @param[in]   aHandle                BCM Handle to retrieve response for
    @param[in]   aReplyID               Reply ID (group, comp, cmd). This could be an identifier for
                                        a set of commands as well.
    @param[in]   aInLen                 Available Length of the reply data payload in bytes
    @param[out]  aReplyData             Pointer to the reply data payload
    @param[out]  aReplyDataLen          Actual Length of the reply data payload in bytes
    @param[out]  aResponse              Response error code for the command processing
    @param[out]  aProgress              Command processing progress in percentage
    ...
    @trace #BRCM_SWREQ_MODULE
*/
typedef int32_t (*MODULE_GetRespV2Type)(BCM_HandleType aHandle,
                                        BCM_MsgType aReplyID,
                                        uint32_t aInLen,
                                        uint8_t *aReplyData,
                                        uint32_t *aReplyDataLen,
...
/** @brief Merge results
    ...
    @trace #BRCM_SWREQ_MODULE
*/
typedef int32_t (*MODULE_MergeResultType)(BCM_MsgType aCmd,
                                          uint32_t aLen,
...
/** @brief Async Notification Message to Host or Component
    ...
    @limitations None
*/
typedef int32_t (*MODULE_AsyncNotifType)(BCM_MsgType aNotfn,
                                        const uint8_t *aMsg,
                                        uint32_t aLen,
...
/** @brief Process the command to the module
    ...
    @trace #BRCM_SWREQ_MODULE
*/
typedef int32_t (*MODULE_ProcessCmdType)(BCM_MsgType aCmd,
...
/** @brief Module notify event handler

    Process the Notification events of the module
    ...
    @param[in]      aHdl            Notification Handle
    @param[in]      aTimestamp      Timestamp at which the notify event occurs
    @param[in]      aCustomData     Custom notification data
    ...
    @trace #BRCM_SWREQ_MODULE
*/
typedef void (*MODULE_NotifyEventHandlerType)(BCM_MsgType aHdl, uint64_t aTimestamp, BCM_NotifyDataType const *aCustomData);
...
/** @brief Get the timer event trigger period

    API to return the timer event trigger period
    (in number of system ticks) needed by the module.
    The event handler of the module is called at this interval
    with MODULE_TimerEvent mask.
    ...
    @retval     #uint32_t   Trigger period in number of ticks
    ...
    @trace #BRCM_SWREQ_MODULE
*/
typedef uint32_t (*MODULE_GetTrigPeriodType)(void);
...
typedef struct sMODULE_ContextType {
    ...
    uint16_t            prof[BCM_STATE_ERROR + 1UL];    /**< @brief profiling information       */
    int32_t             errCode;            /**< @brief Error code preserved temporarily        */
    uint32_t            timerTicks;         /**< @brief Number of ticks since the last timer event */
} MODULE_ContextType;
...
typedef struct sMODULE_Type {
    ...
    MODULE_NotifyEventHandlerType   notifyHandler;     /**< @brief Notification event handler        */
    ...
    MODULE_GetTrigPeriodType        getTrigPeriod;     /**< @brief Get trigger period   */
    ...
    MODULE_GetRespV2Type            getRespV2;         /**< @brief Get the V2 response  */
...
typedef struct sSERVICE_Type {
    ...
    MSGQ_Type                   notifyQ;            /**< @brief Notification Queue */
} SERVICE_Type;

/**
    @brief Maximum number of services in the system

    @trace #BRCM_SWREQ_SERVICE
*/
#define SERVICE_COUNT_MAX   (16UL)

/**
    @brief List of Services in the system

    @trace #BRCM_SWREQ_SERVICE
*/
extern const SERVICE_Type * const SERVICE_List[SERVICE_COUNT_MAX];
