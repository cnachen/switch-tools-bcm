...
#define BRCM_SWARCH_CIRCQ_PEEK_AT_PROC           (0x8911U)    /**< @brief #CIRCQ_PeekAt        */
#define BRCM_SWARCH_CIRCQ_PUSH_AVAILABLE_PROC    (0x8912U)    /**< @brief #CIRCQ_PushAvailable */
/** @} */
...
/** @brief Push objects to Queue.
    ...
    @retval      BCM_ERR_NOMEM           Not enough space to push all the objects
    @retval      BCM_ERR_INVAL_PARAMS    (aQ is NULL ) or
                                         (aData is NULL)
...
/** @brief Push objects to Queue till available size

    This API pushes objects to queue till the space availability.
    ...
    @param[in]    aQ        Pointer to the queue structure.
    @param[in]    aData     Data to push.
    @param[inout] aCnt      In - Number of objects to be pushed. <BR>
                            Out - Actual number of objects pushed
...
*/
extern int32_t CIRCQ_PushAvailable(const CIRCQ_Type *const aQ,
                                   const char *const aData,
                                   uint32_t *aCnt);
...
/** @brief Peeks into objects from Queue at the given offset

    @behavior Sync, Non-reentrant

    @pre None

    This API reads the objects in the circular queue starting from the given offset.
    Read index is not altered.

    @param[in]       aQ          Pointer to the queue structure.
    @param[out]      aData       Pointer to store the objects peeked on queue.
    @param[in]       aOffset     Index offset from where to peak
    @param[in]       aCnt        Number of objects to be peeked on.
    ...
    @limitations None
*/
extern uint32_t CIRCQ_PeekAt(const CIRCQ_Type *const aQ,
                             char *const aData,
                             uint32_t aOffset,
                             uint32_t aCnt);
