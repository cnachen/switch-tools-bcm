...
#define BRCM_SWARCH_BCM_TESTINITMAGIC_MACRO           (0x8716U)    /**< @brief #BCM_TESTQ_INIT_MAGIC      */
...
#define BRCM_SWARCH_BCM_TEST_MSGID_TYPE               (0x871FU)    /**< @brief #BCM_TestMsgIdType         */
...
#define BRCM_SWARCH_BCM_TESTSETSYSTEMSTATE_PROC       (0x872EU)    /**< @brief #BCM_TestSetSystemState    */
#define BRCM_SWARCH_BCM_TESTRUNMAGIC_MACRO            (0x872FU)    /**< @brief #BCM_TESTQ_RUN_MAGIC       */
...
/**
    ...
    @trace #BRCM_SWREQ_BCM_TEST
*/
typedef uint32_t BCM_TestMsgIdType;
#define BCM_TEST_MSG_GROUP_ID_MASK  ((uint32_t)BCM_GROUPID_TEST << BCM_MSG_GROUP_SHIFT) /**< @brief Msg group ID mask  */
#define BCM_TEST_MSG_START_TEST     (0x1U)                                              /**< @brief Test Start Message */
#define BCM_TEST_MSG_IMGL_LOAD      (0x2U | BCM_MSG_ASYNC_MASK)                         /**< @brief Image load async message */
#define BCM_TEST_MSG_CUSTOM_CMD     (0x3U)                                              /**< @brief Test custom command */
/** @} */
...
#define BCM_TESTQ_RESULTQ_FIFO_LOG2_LEN         (7U)
#endif

#define BCM_TESTQ_ROLLOVER_MASK(fifoLenLog2)    ((1 << (fifoLenLog2 + 1)) - 1)
/** @} */
...
/**
    ...
    @trace #BRCM_SWREQ_BCM_TEST
*/
typedef struct sBCM_TestQType {
    uint32_t                initMagic;           /**< @brief Test Init Magic Num */
    uint32_t                runMagic;            /**< @brief Test Ready Magic Num */
...
/**
    ...
    @trace #BRCM_SWREQ_BCM_TEST
*/
#define BCM_TESTQ_INIT_MAGIC  (0x42545354UL)   /**< @brief magic number 'BTST' */

/**
    @name BCM test Q second magic

    @brief Magic word to be placed at next to of BCM Test Q magic

    Until second magic is updated with this value, target is not ready
    for test. Debugger should poll for this magic before starting the test.

    @trace #BRCM_SWREQ_BCM_TEST
*/
#define BCM_TESTQ_RUN_MAGIC  (0x42535352UL)   /**< @brief magic number 'BSSR' */
...
#define BCM_TESTQ_DEFINE_V0(name, major, minor, depth, msgLen, rollMask, hBuf, tBuf) \
    const BCM_TestQType (name) = BCM_TESTQ_INIT((major), (minor), (depth), (msgLen), (rollMask), (hBuf), (tBuf));
...
/** @brief Execute an custom test

    @behavior Async, Non-reentrant

    @pre None

    @param[in]      aCmdMsg      pointor of cmd buffer
    @param[in]      len          cmd length
    ...
    @limitations None
*/
int32_t BCM_ExecuteCustom(uint8_t *aCmdMsg, uint32_t len);
...
/** @brief Load image request
    ...
    @retval     #BCM_ERR_NOMEM          If aBufSize is greater than internal buffer size
...
/** @brief Get status of the image load request
    ...
    @limitations None
*/
int32_t BCM_TestGetLoadImgStatus(MSGQ_HdlType aHdr, uint32_t *const aSize);

/** @brief Notifies that system service state

    This API sets the magic which host polls before starting the test.

    This API should be called from suitable place when system service has
    finished state handling and moving to next state.
    ...
    @limitations None
*/
void BCM_TestSetSystemState(BCM_StateType aState);
