...
/**
    @defgroup grp_crypto_verify_cmd_ifc Verify Send Receive command Interface
    @ingroup grp_verifymod

    @addtogroup grp_crypto_verify_cmd_ifc
    @{

    @limitations None

    @file crypto_verify_cmd.h
    @brief Crypto Verify Send Receive command Interface
    This header file contains the interface functions for crypto based verification
    @version 0.1 Initial Version
*/

#ifndef CRYPTO_VERIRY_CMD_H
#define CRYPTO_VERIRY_CMD_H
...
*/
#define BRCM_SWARCH_CRYPTO_VERIFY_SEND_CMD_PROC     (0x8201U)    /**< @brief #CRYPTO_VerifySendCmd      */
#define BRCM_SWARCH_CRYPTO_VERIFY_GET_RESP_PROC     (0x8202U)    /**< @brief #CRYPTO_VerifyGetResp      */
/** @} */
...
*/
int32_t CRYPTO_VerifySendCmd(BCM_MsgType aCmd, uint32_t aLen, const uint8_t *aCmdData,
...
/** @brief Retreive status of crypto verify commands
    ...
    @trace #BRCM_SWREQ_CRYPTO
*/
int32_t CRYPTO_VerifyGetResp(BCM_HandleType aHandle, BCM_MsgType aReplyID, uint32_t aInLen,
                             uint8_t *aReplyData, uint32_t *aReplyDataLen,
                             int32_t *aResponse, uint32_t *aProgress);
#endif /* CRYPTO_VERIRY_CMD_H */
