...
#define BRCM_SWARCH_BCM_SEARCH_GET_ID_TYPE          (0x8223U)   /**< @brief #BCM_SearchGetIdType    */
#define BRCM_SWARCH_BCM_SEARCH_GET_MATCH_IDX_PROC   (0x8224U)   /**< @brief #BCM_SearchGetMatchIdx  */
/** @} */

/**
    @brief compile time size check assert

    This macro validates the given type size and maximum size of the Type
    This reports an error if type != expectedSz OR
    if the size of the type > maxSz

    @trace #BRCM_SWREQ_BCM_STDLIB_WRAPPER
*/
#define BCM_STATIC_SIZE_ASSERT(type, expectedSz, maxSz)   \
_Static_assert((maxSz) >= sizeof(type), "size of the type is greater than max size!!"); \
_Static_assert((expectedSz) == sizeof(type), "type expected size mismatch!!");
...
/**
    @brief Get message ID function prototype

    @behavior Async, Non-reentrant

    @pre None

    @param[in]  aIndex              Array index to get the message ID

    Return values are documented in reverse-chronological order
    @retval     0UL                 aIndex invalid
    @retval     uint32_t            Valid message ID

    @post None

    @trace #BRCM_SWREQ_BCM_UTILS
*/
typedef uint32_t (*BCM_SearchGetIdType)(uint32_t aIndex);

/** @brief Search and get the match index of message ID

    @behavior Sync, Reentrant

    @pre None

    @param[in]      aSearchVal           The value to search in the array
    @param[in]      aLength              Length of the array
    @param[in]      aFuncPtr             Function pointer to get message ID from array
    @param[inout]   aIdx                 Pointer to store the match index

    Return values are documented in reverse-chronological order
    @retval     #BCM_ERR_OK             aSearchVal found in the array
    @retval     #BCM_ERR_NOT_FOUND      aSearchVal not found in the array
    @retval     #BCM_ERR_INVAL_PARAMS   aLength is 0
    @retval     #BCM_ERR_INVAL_PARAMS   aFuncPtr is NULL
    @retval     #BCM_ERR_INVAL_PARAMS   aIdx is NULL

    @post None

    @note This API is implemented using binary search algorithm. So the array should be populated
          in ascending order of the ID values.

    @trace  #BRCM_SWREQ_BCM_UTILS
*/
int32_t BCM_SearchGetMatchIdx(BCM_MsgType aSearchVal, uint32_t aLength, BCM_SearchGetIdType aFuncPtr, uint32_t * const aIdx);
